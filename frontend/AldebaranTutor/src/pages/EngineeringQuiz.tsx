...
const EngineeringQuiz: React.FC = () => {
    ...
    const fetchQuestion = async (levelName: 'EASY' | 'MEDIUM' | 'HARD') => {
            setIsLoading(true);
            try {
                const response = await fetch(`http://localhost:8080/quiz/engineering?levelName=${levelName}`, {
                    method: 'POST',
                    headers: {
        ...
        return (
            <div className="min-h-screen">
                ...
                <div className="flex flex-col justify-center py-16 px-4 gap-8">
                    <h1 className="text-white text-5xl text-center font-semibold mt-20">Engineering ⚙️</h1>
                    <p className="text-white text-center">Test your knowledge by starting this Engineering Quiz</p>
                </div>
                <div className="flex items-center justify-center">   
                    <Card className="max-w-6xl w-full bg-black border-gray-600 p-8">
                        <CardHeader className="text-white text-left font-bold text-2xl">
                            Ready to test your Engineering knowledge?
                            <CardDescription className="text-white text-base">The quiz contains 10 questions based on EASY, MEDIUM, HARD LEVEL in Engineering sectors. Please select the level and make sure you answer correctly. Good luck!</CardDescription>
                        </CardHeader>
